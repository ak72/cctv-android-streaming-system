...
*   **Solution**: Recording uses either **SAF** (user-selected folder via `video_tree_uri`) or **MediaStore** with `RELATIVE_PATH` (e.g. `Movies/CCTV`). `CustomRecorder` writes via `MediaMuxer(FileDescriptor)` to a file created by the service (SAF or MediaStore). See `VIDEO_RECORDING.md` and `AppSettings.DEFAULT_VIDEO_RELATIVE_PATH`.
