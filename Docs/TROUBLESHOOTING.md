...
### No audio when starting recording from the Viewer
*   **Symptom**: Recording started via the Viewer app saves video but no audio.
*   **Cause**: Streaming audio (and thus `AudioSourceEngine` capture) was only started when the camera bind callback ran (e.g. when the Primary UI had been in foreground). When the user started recording only from the Viewer, capture might not have been running when `CustomRecorder` registered as a recording listener.
*   **Fix**: The Primary now calls `startStreamingAudio()` before `startRecording(true)` when handling remote `START_RECORDING`. This ensures the mic is capturing before the recorder registers, so the file includes audio. No workaround needed on current builds.
