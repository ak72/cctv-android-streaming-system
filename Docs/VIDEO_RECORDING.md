...
### D. Camera Switch During Recording
*   When the user switches the camera (Primary or Viewer) while recording, the pipeline rebinds to the new camera. The **same** `CustomRecorder` instance is kept; after each successful bind, the service re-applies the recording sink on the (possibly recreated) encoder so that the single MP4 file captures the **full duration** without being truncated at the switch.
...
1.  **Start**:
    ...
    *   OR: Send a `START_RECORDING` command remotely from `CCTVViewer`. When started from the Viewer, the Primary ensures streaming audio is started before the recorder so the saved file includes **audio** as well as video.
2.  **During Recording**:
    ...
    *   **Camera switch** is supported: the user may switch between front and back camera on either the Primary or the Viewer; the same recording continues for the **full duration** (the recording sink is re-applied after camera rebind).
3.  **Stop**:
