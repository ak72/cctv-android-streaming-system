...
*   **Switch Camera**: Seamless tear-down and re-bind of the pipeline to switch between Front and Back lenses. Supported **during recording**; the same recording continues for the full duration (the recording sink is re-applied after rebind). Both Primary and Viewer UIs allow switching the camera while streaming or recording.
*   **Low Power Mode**:
