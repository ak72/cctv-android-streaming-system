package com.anurag.cctvviewer.ui.responsive
...
fun ResponsiveSingleScreen(
    ...
    val providedDensity: Density =
        remember(baseDensity.density, config.fontScale, safeFontScaleRange) {
            if (safeFontScaleRange == null) {
                baseDensity
            } else {
                val safeFontScale = config.fontScale.coerceIn(
                    safeFontScaleRange.start, safeFontScaleRange.endInclusive
                )
    ...
    BoxWithConstraints(
        ...
        LaunchedEffect(usableW, usableH, refWidth, refHeight, finalScale, config.fontScale) {
            Log.d(
                logTag,
                "🔵 [RESPONSIVE_UI] ref=${refWidth.value}x${refHeight.value}dp usable=${usableW}x${usableH}dp " + "rawScale=$rawScale finalScale=$finalScale density=${baseDensity.density} fontScale=${config.fontScale} "
            )
        }
