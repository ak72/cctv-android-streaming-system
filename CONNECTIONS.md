...
Key characteristics:
*   **Protocol**: Custom line-based TCP protocol (ASCII commands for control, Binary for data).
*   **Port**: Dynamically assigned (defaults to `8080`, handled by Android OS).
...
1.  **Viewer**: Sends `HELLO|client=viewer|version=1`
...
2.  **Primary (`StreamServer`)**:
    ...
    *   **Override**: If in "Buffer Mode", the Primary forces the resolution to match the encoder (e.g., 960x720).
...
*   **State**: `DISCONNECTED` -> `AUTHENTICATED` -> `STREAMING` -> `RECONFIGURING`.
*   **Queues**:
...
*   **Timeout**: If no `PING` is received for **~20 seconds**, `ViewerSession` closes the connection (tolerates brief scheduling stalls/background transitions).
...
## 4. Connection States
The `StreamState` enum guides the logic:
1.  **DISCONNECTED**: Initial state or closed.
2.  **AUTHENTICATED**: Password verified, waiting for CAPS.
3.  **STREAMING**: Normal operation. Frames are flowing.
4.  **RECONFIGURING**: Encoder is restarting (changing bitrate/resolution). Video flow is paused/dropped until new Keyframe.
